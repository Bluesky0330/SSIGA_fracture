...
double Make_B_component_for_SSIGA(int ele_disp, double *para_disp, double *out_B_component, information *info)
{
	double J = 0.0;
	double a_2x2[4], a_3x3[9];
	...
	else if (info->DIMENSION == 3)
	{
		for (int i = 0; i < info->DIMENSION; i++)
		{
			...
			{
				a_3x3[i * 3 + j] = 0.0;
				for (int k = 0; k < info->Geo_No_Control_point_ON_ELEMENT[info->Element_patch[ele_geo]]; k++)
				{
					int id = info->Geo_Controlpoint_of_Element[ele_geo * MAX_NO_CP_ON_ELEMENT + k] * (info->DIMENSION + 1) + i;
