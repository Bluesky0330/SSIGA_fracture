...
不明 Ver.2.1 荒井作成
2021/12 Ver2.2_pipe_moving 合田作成
--------------------------------------------------------
...
積分領域が形成されないバグを修正
各節点のq値を出力する
亀裂前縁節点のCTOA(亀裂先端開口角)を出力する(必要に応じてMake_CTOA_coordinateを書き換えること)
...
Argv[5]:FEM_result_data	(~~~_JI_InputFEMResultData.dat)
...
Argv[16]:CTOA_result_name
Argv[17]:q2_result_name
...
static int Flag_Vep_Nodes[MAX_N_NODE];
...
static int CTOA_node[MAX_NODES_CRACK_FRONT][4];
static double CTOA_node_coordinate0[MAX_NODES_CRACK_FRONT][3];
static double CTOA_node_coordinate1[MAX_NODES_CRACK_FRONT][3];
static double Constant_term[MAX_NODES_CRACK_FRONT];
static double Distance1[MAX_NODES_CRACK_FRONT];
static double Distance2[MAX_NODES_CRACK_FRONT];
static double CTOA[MAX_NODES_CRACK_FRONT];
...
double ComputeValueQFunc(double x, double y, double z,
			 ...
			 double r0, double r1,double R,double Rc,double xr)
{
	...
	double xt, yt;
  	...
  	zzabs = fabs(R-Rc); //径方向距離
	xt = xr; //周方向距離
	yt = yy; //亀裂法線方向距離
  	rr = sqrt(xt*xt + yt*yt); //配管形状を考慮した距離
	...
	*******************************************************************/
  	...
  	if(rr <= r0 + Eps && yy < r0 + Eps){
  		...
  		else if( z1 - Eps < zzabs ){
  			qfunc = 0.0;
  			Search_AreaFlag  = 7;     //7
  		}
  	}
  	else if( (r0 < rr) && (rr <= r1 + Eps) && yy < r0 + Eps){
  		if( zzabs <= z0 ){
  			qfunc = (r1 - rr) / (r1 - r0);        //②
  			Search_AreaFlag =2;
  		}
  		...
  		else if( (z0 + z1 + (rr-r0)/(r1-r0) * z2 - Eps) < zzabs){
  			qfunc = 0.0;
  			Search_AreaFlag =6;    //soto
  		}
  	}
  	else if(r1 < rr){
  		qfunc =0.0;
  	...
  	if(qfunc> Eps && rr < Eps){
    		//printf("xx yy zz zzabs q %10.3e %10.3e %10.3e %10.3e %10.3e\n",xx,yy,zz,zzabs,qfunc);//printf("xc yc zc z %10.3e %10.3e %10.3e %10.3e\n",xc,yc,zc,z);
  	}
  return(qfunc);
}
...
int PerformIntegralonElement_tetra(int elementId, int nodes_count,int nn){
  	...
  	for(ii=0;ii<nodes_count;ii++){
    		...
    		else if(Flag_Vep_Nodes[ElementNodeId_s[elementId][ii]]==2) CountVepFlag++;
    		if(Flag_Vep_Nodes[ElementNodeId_s[elementId][ii]]==3) Count_OutVepFlag++;
...
int PerformIntegralonElement_Hexa(int elementId, int nodes_count,int nn){
  	...
  	for(ii=0;ii<nodes_count;ii++){
    		...
    		if(Flag_Vep_Nodes[ElementNodeId_s[elementId][ii]]==0 || Flag_Vep_Nodes[ElementNodeId_s[elementId][ii]]==3) Count_OutFlag++;
    	}
  	if(icount == nodes_count) return(0);  		
  	else{
		if(CountVepFlag == nodes_count){
			//printf("Ele %d CountVep %d |Outer %d %d |Integ %d\n",elementId,CountVepFlag,icount,Count_OutFlag,Integcount );
			return(2);
		}
		else if(CountVepFlag == N_Face_Nodes){
			if(Count_OutFlag == N_Face_Nodes){
				return(2);
			}
			else return(1);
		}
...
//q2の勾配を求める関数
void Make_gradients_q2_Tetra_10(int cfn, int nn, double RR, double temp_z1, double temp_z2){
  	...
  	for( i = 0; i < NNodes; i++ )
  	{
    		...
    		q2[i][nn] = 0.0;
    		q2[i][nn]  = ComputeValueQFunc(xx,  yy,  zz, z0,  z2, z1,xxc, yyc, zzc, r0, r1,R,Rc,xr);    //すべての節点に対してq値が入る。

    	//	if(Search_AreaFlag==7)printf("7777777777777______");
    		if(q2[i][nn] >0){
    			if(Search_AreaFlag==1 || Search_AreaFlag==3){
    				Flag_Vep_Nodes[i]=2;
    			}
    			else if(Search_AreaFlag==2 || Search_AreaFlag==4 || Search_AreaFlag==5){
    				Flag_Vep_Nodes[i]=1;
    			}
...
//q2の勾配を求める関数
void Make_gradients_q2_Tetra_4(int cfn, int nn, double RR, double temp_z1, double temp_z2){
...
//q2の勾配を求める関数
void Make_gradients_q2_Hexa_8(int cfn, int nn, double RR, double temp_z1, double temp_z2){
...
//この部分を配管の形状を考慮するように設定した。必要に応じて他の関係する変数も書き換えること(配管の向きや要素の大きさ、形状に従って変更する必要がある)////////////////
			theta = fabs(acos((xc*x+yc*y)/((xc*xc+yc*yc)*(x*x+y*y)))); //配管の断面がxy面で長手方向がz方向のとき、亀裂前縁上の基準となる節点と探索節点の角度を求めている
			
			R = sqrt(x*x+y*y); //xy面中心と探索節点の距離
			Rc = sqrt(xc*xc+yc*yc); //xy面中心と基準節点の距離
			xa=x*Rc/R; //探索節点を基準節点と同じ径方向距離に移動した時のx座標
			ya=y*Rc/R; //探索節点を基準節点と同じ径方向距離に移動した時のy座標

			xr=sqrt((xa-xc)*(xa-xc)+(ya-yc)*(ya-yc))*theta/(2*fabs(sin(theta/2))); //配管の形状に沿った周方向距離 
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    		//Converting to local coords  局所座標系に座標変換
    		xx = ConvLocalCoords(localCoord0, x, y, z);
    		yy = ConvLocalCoords(localCoord1, x, y, z);
...
void Make_J_Integral_Tetra_10(const char *fileName, char *code1, char *code2, char *code3){
	...
	/*各項の数値を求めているパート*/
  	...
  	for(n=0;n<Ncrack_node;n++){
  		...
  		Make_gradients_q2_Tetra_10(crack_node[n], n, RR, temp_z1, temp_z2);
  		dA = Compute_dA(n);
...
void Make_J_Integral_Hexa_8(const char *fileName, char *code1, char *code2, char *code3){
	...
	/*各項の数値を求めているパート*/
  	...
  	for(n=0;n<Ncrack_node;n++){
  		...
  		Make_gradients_q2_Hexa_8(crack_node[n], n, RR, temp_z1, temp_z2);
  		dA = Compute_dA(n);
  		//printf("dA=%lf\n",dA );
  		Make_P_2j_gradients_q2();
  		Make_second_eq();
...
void Make_J_Integral_Tetra_4(const char *fileName, char *code1, char *code2, char *code3){
	...
	/*各項の数値を求めているパート*/
  	...
  	for(n=0;n<Ncrack_node;n++){
  		...
  		Make_gradients_q2_Tetra_4(crack_node[n], n, RR, temp_z1, temp_z2);
  		dA = Compute_dA(n);
  		Make_P_2j_gradients_q2();
  		Make_second_eq();
...
void Make_CTOA_coordinate(){

	int n,nn,i,cfn;
	for(nn=0;nn<Ncrack_node + 2;nn++){
		if(nn==0){
			cfn = CrackFrontSegments_For_Jint[0][0][0];
		}
		else{
			cfn = CrackFrontSegments_For_Jint[0][nn-1][1];
		}
		...
		if(nn == 0){
			for(kk=0; kk<3; kk++){
      			localCoord0[kk] = LocalCoodVecsCF[0][nn][0][0][kk] ;  //進展方向ベクトル
      			localCoord1[kk] = LocalCoodVecsCF[0][nn][0][1][kk]; 	//法線方向ベクトルの3成分作成
      			localCoord2[kk] = LocalCoodVecsCF[0][nn][0][2][kk];  //接線方向ベクトル
    		}
		}
		else if(nn == Ncrack_node){
			for(kk=0; kk<3; kk++){
      			localCoord0[kk] = LocalCoodVecsCF[0][nn-1][1][0][kk] ;  //進展方向ベクトル
      			localCoord1[kk] = LocalCoodVecsCF[0][nn-1][1][1][kk]; 	//法線方向ベクトルの3成分作成
      			localCoord2[kk] = LocalCoodVecsCF[0][nn-1][1][2][kk];  //接線方向ベクトル
				
    		}
		}
			else{
    			for(kk=0; kk<3; kk++){
      				localCoord0[kk] = ( LocalCoodVecsCF[0][nn-1][1][0][kk] + LocalCoodVecsCF[0][nn][0][0][kk])*0.5;  //進展方向ベクトル
      				localCoord1[kk] = ( LocalCoodVecsCF[0][nn-1][1][1][kk] + LocalCoodVecsCF[0][nn][0][1][kk])*0.5; 	//法線方向ベクトルの3成分作成
      				localCoord2[kk] = ( LocalCoodVecsCF[0][nn-1][1][2][kk] + LocalCoodVecsCF[0][nn][0][2][kk])*0.5;  //接線方向ベクトル
    			}
			}	
			...
			if(fabs(zz-zzc) < av_d*0.1 &&  fabs(xx-xxc) < av_d*1.5 && xxc-xx > 0 && fabs(yy-yyc) < av_d*0.1){ //CTOAの節点を探す条件。初期配置の要素サイズに合わせて変更して下さい
            
				if(CTOA_node[nn][1] = -1 ){
					CTOA_node[nn][1] = i;  
					Constant_term[nn] = localCoord1[0] * xc + localCoord1[1] * yc + localCoord1[2] * zc ;
            	}
				else if(CTOA_node[nn][1] != -1){
					CTOA_node[nn][2] = i; 
				}
        	}

			CTOA_node_coordinate0[nn][0] = Node_Coordinate[CTOA_node[nn][0]][0] + Displacement[CTOA_node[nn][0]][0];
			CTOA_node_coordinate0[nn][1] = Node_Coordinate[CTOA_node[nn][0]][1] + Displacement[CTOA_node[nn][0]][1];
			CTOA_node_coordinate0[nn][2] = Node_Coordinate[CTOA_node[nn][0]][2] + Displacement[CTOA_node[nn][0]][2];

			CTOA_node_coordinate1[nn][0] = Node_Coordinate[CTOA_node[nn][1]][0] + Displacement[CTOA_node[nn][1]][0];
			CTOA_node_coordinate1[nn][1] = Node_Coordinate[CTOA_node[nn][1]][1] + Displacement[CTOA_node[nn][1]][1];
			CTOA_node_coordinate1[nn][2] = Node_Coordinate[CTOA_node[nn][1]][2] + Displacement[CTOA_node[nn][1]][2];

			Distance1[nn] = fabs(localCoord1[0]*CTOA_node_coordinate1[nn][0] + localCoord1[1]*CTOA_node_coordinate1[nn][1] + localCoord1[2]*CTOA_node_coordinate1[nn][2] - Constant_term[nn])/sqrt(localCoord1[0]*localCoord1[0]+localCoord1[1]*localCoord1[1]+localCoord1[2]*localCoord1[2]);
			Distance2[nn] = sqrt((CTOA_node_coordinate1[nn][0]-CTOA_node_coordinate0[nn][0])*(CTOA_node_coordinate1[nn][0]-CTOA_node_coordinate0[nn][0]) +(CTOA_node_coordinate1[nn][1]-CTOA_node_coordinate0[nn][1]) * (CTOA_node_coordinate1[nn][1]-CTOA_node_coordinate0[nn][1])+(CTOA_node_coordinate1[nn][2]-CTOA_node_coordinate0[nn][2]) * (CTOA_node_coordinate1[nn][2]-CTOA_node_coordinate0[nn][2]));
	...
	for(n=0;n<Ncrack_node+2;n++){
		CTOA[n]=asin(Distance1[n]/Distance2[n]); 
	}
}
...
void Output_CTOA_coordinate(const char *fileName){
	FILE *fp;
  	fp = fopen(fileName, "w");
	int i,j,n;
...
for(n=0;n<Ncrack_node+2;n++){
	...
	fprintf(fp,"%d\n",n);
	fprintf(fp,"%d	%d\n",CTOA_node[n][0],CTOA_node[n][1]);
	fprintf(fp,"%lf	%lf	%lf\n",CTOA[n]*2,Distance1[n]*2,Distance2[n]*cos(CTOA[n]));
	fprintf(fp,"%lf	%lf	%lf\n",CTOA_node_coordinate0[n][0],CTOA_node_coordinate0[n][1],CTOA_node_coordinate0[n][2]);
	fprintf(fp,"%lf	%lf	%lf\n",CTOA_node_coordinate1[n][0],CTOA_node_coordinate1[n][1],CTOA_node_coordinate1[n][2]);
...
int main (int argc, char *argv[])
{
	...
	reading_J_DI_input_data(argv[3]);
	...
	Make_CTOA_coordinate();
    Output_CTOA_coordinate(argv[16]);
