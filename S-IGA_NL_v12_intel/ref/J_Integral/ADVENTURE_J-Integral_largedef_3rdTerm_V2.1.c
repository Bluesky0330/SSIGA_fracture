			 ...
			 double r0, double r1)
{
	...
	*******************************************************************/
  	...
  	if(rr <= r0 + Eps){
  		...
  		else if( (z0 < zzabs) && (zzabs <= z0 + (rr-r0)/(r1-r0) * z2 + Eps)){
  			qfunc = (r1 - rr) / (r1 - r0);        //④
  			Search_AreaFlag =4;
  		}
  		else if( (z0 + (rr-r0)/(r1-r0) * z2) < zzabs && zzabs <= (z0 + z1 + (rr-r0)/(r1-r0) * z2 +Eps) ){
...
int PerformIntegralonElement_tetra(int elementId, int nodes_count){
  	int ii,icount=0,CountVepFlag=0,Count_OutVepFlag=0;
  	double Eps=0.00000001;
  	int temp2layer=0;
  	for(ii=0;ii<nodes_count;ii++){
    		if(fabs(q2[ElementNodeId_s[elementId][ii]]) < Eps) icount++;
...
/*探索する要素のコネクティビティに含まれる節点のq値がすべて0の時0を返す→積分領域外
  ...
  それ以外→１を返す*/
int PerformIntegralonElement_Hexa(int elementId, int nodes_count){
...
void Make_gradients_q2_Tetra_10(int cfn, int nn, double RR, double temp_z1, double temp_z2){
  	...
  	for( i = 0; i < NNodes; i++ )
  	{
    		...
    		q2[i] = 0.0;
    		q2[i]  = ComputeValueQFunc(xx,  yy,  zz, z0,  z2, z1,xxc, yyc, zzc, r0, r1);    //すべての節点に対してq値が入る。
  	...
  	for( e = 0; e < NElements; e++ ){
			...
			if(isnan(JW_on_Node[ElementNodeId_s[e][i]][0]) != 0){
	    			for( i = 0; i < NO_NODES_ON_ELEMENT_T10; i++ ){
					q2[ElementNodeId_s[e][i]] = 0.0;
					q2_dd[i] = q2[ ElementNodeId_s[e][i] ];
				}
			}
    		...
    		q2[ ElementNodeId_s[e][4] ] = q2_dd[4];
    		q2[ ElementNodeId_s[e][5] ] = q2_dd[5];
    		q2[ ElementNodeId_s[e][6] ] = q2_dd[6];
    		q2[ ElementNodeId_s[e][7] ] = q2_dd[7];
    		q2[ ElementNodeId_s[e][8] ] = q2_dd[8];
    		q2[ ElementNodeId_s[e][9] ] = q2_dd[9];
    		...
    		for( N = 0; N < N_IntegrationPoint; N++ ){
      		q2_gp[e][N]=0.0;
      		for( i = 0; i < NO_NODES_ON_ELEMENT_T10; i++ ){
				q2_gp[e][N]+=N_Tetra_10(i, Gxi[N])*q2[ElementNodeId_s[e][i]];
      		}
    		}
  	}
}
	...
	double Gxi[1][3]={0.25,0.25,0.25};
  	...
  	for( e = 0; e < NElements; e++ ){
    		...
    		for( N = 0; N < N_IntegrationPoint; N++ ){
      		q2_gp[e][N]=0.0;
      		for( i = 0; i < NO_NODES_ON_ELEMENT_T4; i++ ){
				q2_gp[e][N]+=N_Tetra_4(i, Gxi[N])*q2[ElementNodeId_s[e][i]];
      		}
    		}
  	}
}
	...
	double Gxi[8][3]=	{{-0.57735026918962584208,-0.57735026918962584208,-0.57735026918962584208},
  	...
  	for( e = 0; e < NElements; e++ ){
    		...
    		for( N = 0; N < N_IntegrationPoint; N++ ){
      		q2_gp[e][N]=0.0;
      		for( i = 0; i < NO_NODES_ON_ELEMENT_H8; i++ ){
				q2_gp[e][N]+=N_Hexa_8(i, Gxi[N])*q2[ElementNodeId_s[e][i]];
      		}
    		}
  	}
}
...
void Make_J_Integral_Tetra_10(const char *fileName, char *code1, char *code2, char *code3){
	...
	/*各項の数値を求めているパート*/
  	...
  	for(n=0;n<Ncrack_node;n++){
  		...
  		int count_int=0;
  		for( e = 0; e < NElements; e++ ){
  			temp_PerformFlag=PerformIntegralonElement_tetra(e,Number_of_Nodes_on_Elements);
...
void Make_J_Integral_Hexa_8(const char *fileName, char *code1, char *code2, char *code3){
	...
	/*各項の数値を求めているパート*/
  	...
  	for(n=0;n<Ncrack_node;n++){
  		...
  		int count_int=0;
  		for( e = 0; e < NElements; e++ ){
  			temp_PerformFlag=PerformIntegralonElement_Hexa(e,Number_of_Nodes_on_Elements);
    			if(temp_PerformFlag > 0){
      			//各要素の変位を取得
      			for( i = 0; i < NO_NODES_ON_ELEMENT_H8; i++ ){
