...
2022/12/10 Ver.2.2_elongating 合田作成
--------------------------------------------------------
...
Argv[1]:.msh
Argv[2]:.fgr
Argv[3]:現在亀裂のJ_DI_input_data	(temp_J_int.dat)
...
Argv[16]:初期亀裂のJ_DI_input_data	(temp_J_int.dat) 
...
double ComputeValueQFunc(double x, double y, double z,
			 ...
			 double r0, double r1,double xc2, double yc2, double zc2,int i,int n)
{
	...
	double rr2, zzabs2;
	double a0, b0, c0; //亀裂前縁を繋ぐ直線の係数(xy)
	double Disp0; //サーチ点と亀裂前縁を繋ぐ直線の距離(xy)
	double a1, b1, c1; //亀裂前縁を繋ぐ直線の係数(xz)
  	double qfunc=0.0;
	...
	b0 = -1;
	c0 = yc - (yc2 - yc)/(xc2 - xc);
	Disp0 = fabs(a0 * x + b0 * y + c0)/sqrt(a0 * a0 + b0 * b0);
...
void Make_gradients_q2_Tetra_10(int cfn, int nn, double RR, double temp_z1, double temp_z2,int cfn2){
  	...
  	for( i = 0; i < NNodes; i++ )
  	{
    		...
    		q2[i][nn] = 0.0;
    		q2[i][nn]  = ComputeValueQFunc(xx,  yy,  zz, z0,  z2, z1,xxc, yyc, zzc, r0, r1, xxc2, yyc2, zzc2, i,nn);    //すべての節点に対してq値が入る。
