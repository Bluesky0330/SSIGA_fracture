...
void searchOverlappingEle(information *info)
{
	...
	for (int e = info->Total_Element_to_mesh[1]; e < info->Total_Element_to_mesh[Total_mesh]; e++)
	{
		...
		for (int i = 0; ; i++)
		{
			...
			if (i == 0 && !checkInner(td_base, td[0], info) && eoi[e].size() == 0)
			{
...
void physical_coord(const int e, double *para, double *out_coord, information *info)
{
	...
	{
		double R = Shape_func(i, para, e, info);
		for (int j = 0; j < info->DIMENSION; j++)
			out_coord[j] += R * info->Node_Coordinate[info->Controlpoint_of_Element[e * MAX_NO_CP_ON_ELEMENT + i] * (info->DIMENSION + 1) + j];
	}
	return;
}
...
bool search2D(target_domain &td_base, target_domain &td, information *info)
{
	...
	static double *R_base_parallel = (double *)malloc(sizeof(double) * MAX_NO_CP_ON_ELEMENT * omp_get_max_threads());
	static double *R_parallel = (double *)malloc(sizeof(double) * MAX_NO_CP_ON_ELEMENT * omp_get_max_threads());
	static double *dR_base_parallel = (double *)malloc(sizeof(double) * MAX_DIMENSION * MAX_NO_CP_ON_ELEMENT * omp_get_max_threads());
	static double *dR_parallel = (double *)malloc(sizeof(double) * MAX_DIMENSION * MAX_NO_CP_ON_ELEMENT * omp_get_max_threads());
	double *R_base = &R_base_parallel[MAX_NO_CP_ON_ELEMENT * omp_get_thread_num()];
	double *R = &R_parallel[MAX_NO_CP_ON_ELEMENT * omp_get_thread_num()];
	double *dR_base = &dR_base_parallel[MAX_DIMENSION * MAX_NO_CP_ON_ELEMENT * omp_get_thread_num()];
	...
	for (int i = 0; i < edge_n; i++)
	{
		for (int j = 0; j < edge_n; j++)
		{
			...
			for (int k = 0; k < max_itr; k++)
			{
				...
				shape_and_dshape(R_base, dR_base, tilde_para_base, e_base, info);
				shape_and_dshape(R, dR, tilde_para, e, info);
				physical_coord_R(e_base, R_base, coord_base, info);
				physical_coord_R(e, R, coord, info);
...
void setEdge3D(int edge_or_face, int edge_or_face_num, int *target_axis, double *para, target_domain &temp)
{
	...
	else if (edge_or_face == 1)
	{
		if (edge_or_face_num == 0)
		{
			...
			para[0] = temp.para_start[0];
			para[1] = (temp.para_start[1] + temp.para_end[1]) * 0.5;
			para[2] = (temp.para_start[1] + temp.para_end[1]) * 0.5;
		}
		else if (edge_or_face_num == 1)
		{
			target_axis[0] = 1, target_axis[1] = 2;
