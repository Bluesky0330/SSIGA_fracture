...
void Modify_D_matrix(double *D, double *current_stress, double *elastic_strain_trial, double *current_deformation_gradients);
...
double blendingFunction(const int e, const int p, double *para, information *info);
#if 0
...
void findAxisIntersection(vector<double> &result, const vector<double> &o, const vector<double> &a, const int dim);
...
void Update_Control_Point_glo_using_L2_projection_and_aux(information *info);
...
void identify_tensor(int rank, double *A);
double tensor_norm(int rank, double *A);
void tensor_exponent(int rank, double *A);
void tensor_logarithm(int rank, double *A);
