...
void searchOverlappingEle(information *info)
{
	...
	vector<vector<subcell>> &octree_subcell = info->octree_subcell; // octree
	...
	for (int e = info->Total_Element_to_mesh[1]; e < info->Total_Element_to_mesh[Total_mesh]; e++)
	{
		...
		td_base.setPara(0, zero_vec, info); // consider octree level and position to be 0
		...
		for (int i = 0; ; i++)
		{
			...
			{
				td[j].setPara(0, zero_vec, info); // consider octree level and position to be 0
	...
	// make octree
	if (info->c.MAX_OCTREE_N > 0)
	{
		vector<octree_root> tree(info->Total_Element_to_mesh[Total_mesh]);
		int current_division = pow_int(2, info->DIMENSION);
		#pragma omp parallel for schedule(dynamic)
		for (int e = 0; e < info->Total_Element_to_mesh[Total_mesh]; e++)
		{
			// make root octree
			octree_node *root = new octree_node(zero_vec, 0, info);
			tree[e].root = root;
			for (size_t i = 0; i < eoi[e].size(); i++)
				tree[e].root->overlapping_ele.emplace_back(eoi[e][i]);
			target_domain root_td(e);
			root_td.setPara(0, zero_vec, info);
			tree[e].root->recieve_para(root_td, info);

			// skip if single overlapping element
			if (eoi[e].size() <= 1)
				continue;

			vector<octree_node *> current_level_vec, next_level_vec;
			current_level_vec.emplace_back(tree[e].root);

			// make child octree
			for (int octree_level = 1; octree_level <= info->c.MAX_OCTREE_N; octree_level++)
			{
				next_level_vec.clear();

				// search stack
				for (size_t i = 0; i < current_level_vec.size(); i++)
				{
					octree_node *current = current_level_vec[i];

					// check if leaf
					if (current->is_leaf && current->overlapping_ele.size() <= 1)
						continue;
					else
						current->is_leaf = false;

					// make child
					for (int j = 0; j < current_division; j++)
					{
						int child_position[MAX_DIMENSION];
						makeChildPosition(child_position, j, current->octree_position, info);
						octree_node *child = new octree_node(child_position, octree_level, info);
						current->children.emplace_back(child);

						target_domain td_base(e);
						td_base.setPara(octree_level, child_position, info);
						current->children[j]->recieve_para(td_base, info);

						// set overlapping element
						for (size_t k = 0; k < current->overlapping_ele.size(); k++)
						{
							target_domain td(current->overlapping_ele[k]);
							td.setPara(0, zero_vec, info);

							if (checkInnerCenterEachPatch(td, td_base, info) || checkInnerCenterEachPatch(td_base, td, info))
							{
								current->children[j]->overlapping_ele.emplace_back(td.e);
								continue;
							}

							auto overlapping = search(td_base, td, info);
							if (overlapping)
								current->children[j]->overlapping_ele.emplace_back(td.e);
						}

						// set next level
						next_level_vec.emplace_back(current->children[j]);
					}
				}

				// swap
				std::swap(current_level_vec, next_level_vec);
			}
		}
		...
		for (int e = 0; e < info->Total_Element_to_mesh[Total_mesh]; e++)
		{
			vector<octree_node *> stack;
			stack.emplace_back(tree[e].root);

			while (!stack.empty())
			{
				octree_node *current = stack.back();
				stack.pop_back();

				if (current->is_leaf)
				{
					octree_subcell[e].emplace_back(e, *current, info);
					continue;
				}

				for (size_t i = 0; i < current->children.size(); i++)
					stack.emplace_back(current->children[i]);
			}
		}
	}
...
int setStartEle(const int e, information *info)
{
	...
	physical_coord(e, para, out_coord, info);

	// search global point
	int itr_n = 0, g_p = 0;
	for (int i = 0; i < info->Total_Patch_to_mesh[1]; i++)
	{
		itr_n = calc_patch_parameter_coord(out_coord, i, out_para, info);
		if (itr_n != ERROR)
		{
			g_p = i;
			break;
	...
	if (check_nonfinite(out_para, info->DIMENSION))
		return ERROR;
...
bool checkInner(target_domain &td_base, target_domain &td, information *info)
{
	...
	double out_coord[MAX_DIMENSION] = {0.0};
	double out_para[MAX_DIMENSION] = {0.0};
	physical_coord(td.e, vertex_para, out_coord, info);
...
bool checkInnerCenter(target_domain &td_base, int e_center, information *info)
{
	...
	double out_coord[MAX_DIMENSION] = {0.0};
	double out_para[MAX_DIMENSION] = {0.0};
	physical_coord(e_center, center_para, out_coord, info);
...
bool checkInnerCenterEachPatch(target_domain &td_base, target_domain &td, information *info)
{
	...
	double center_tilde_para[MAX_DIMENSION] = {0.0};
	double out_coord[MAX_DIMENSION] = {0.0};
	double out_para[MAX_DIMENSION] = {0.0};
	
	tilde_coord(center_tilde_para, center_para, info->Element_patch[td.e], td.e, info);
	physical_coord(td.e, center_tilde_para, out_coord, info);
...
bool search2D(target_domain &td_base, target_domain &td, information *info)
{
	...
	for (int i = 0; i < edge_n; i++)
	{
		for (int j = 0; j < edge_n; j++)
		{
			...
			int target_axis = 0;
			int patch_base = info->Element_patch[td_base.e];
			int patch = info->Element_patch[td.e];
			...
			for (int k = 0; k < max_itr; k++)
			{
				...
				if (check_nonfinite(para_base, info->DIMENSION) || check_nonfinite(para, info->DIMENSION))
					break;
				int e_base = ele_check(patch_base, para_base, info);
				int e = ele_check(patch, para, info);
				tilde_coord(tilde_para_base, para_base, patch_base, e_base, info);
				tilde_coord(tilde_para, para, patch, e, info);
				shape_and_dshape(R_base.data(), dR_base.data(), tilde_para_base, e_base, false, info);
				shape_and_dshape(R.data(), dR.data(), tilde_para, e, false, info);
				physical_coord_R(e_base, R_base.data(), coord_base, info);
				physical_coord_R(e, R.data(), coord, info);
				...
				// make J
				for (int l = 0; l < info->DIMENSION; l++)
					for (int n = 0; n < info->No_Control_point_ON_ELEMENT[info->Element_patch[e_base]]; n++)
						J[l * info->DIMENSION + 0] += dR_base[n * info->DIMENSION + target_axis_base] * info->Node_Coordinate[info->Controlpoint_of_Element[e_base * MAX_NO_CP_ON_ELEMENT + n] * (info->DIMENSION + 1) + l];
				for (int l = 0; l < info->DIMENSION; l++)
					for (int n = 0; n < info->No_Control_point_ON_ELEMENT[info->Element_patch[e]]; n++)
						J[l * info->DIMENSION + 1] -= dR[n * info->DIMENSION + target_axis] * info->Node_Coordinate[info->Controlpoint_of_Element[e * MAX_NO_CP_ON_ELEMENT + n] * (info->DIMENSION + 1) + l];
			...
			if (!singular_flag)
			{
				...
				int e_base = ele_check(patch_base, para_base, info);
				tilde_coord(tilde_para_base, para_base, patch_base, e_base, info);
				physical_coord(e_base, tilde_para_base, coord_base, info);
				...
				for (int k = 0; k < max_itr; k++)
				{
					...
					// make J
					for (int n = 0; n < info->No_Control_point_ON_ELEMENT[info->Element_patch[e]]; n++)
						J -= dR[n * info->DIMENSION + target_axis] * info->Node_Coordinate[info->Controlpoint_of_Element[e * MAX_NO_CP_ON_ELEMENT + n] * (info->DIMENSION + 1) + singular_axis];
				...
				int e = ele_check(patch, para, info);
				tilde_coord(tilde_para, para, patch, e, info);
				physical_coord(e, tilde_para, coord, info);
				...
				for (int k = 0; k < max_itr; k++)
				{
					...
					// make J
					for (int n = 0; n < info->No_Control_point_ON_ELEMENT[info->Element_patch[e_base]]; n++)
						J += dR_base[n * info->DIMENSION + target_axis_base] * info->Node_Coordinate[info->Controlpoint_of_Element[e_base * MAX_NO_CP_ON_ELEMENT + n] * (info->DIMENSION + 1) + singular_axis];
...
bool search3D(target_domain &td_base, target_domain &td, information *info)
{
			...
			for (int k = 0; k < max_itr; k++)
			{
				...
				for (int l = 0; l < 2; l++)
					for (int m = 0; m < info->DIMENSION; m++)
						for (int n = 0; n < info->No_Control_point_ON_ELEMENT[info->Element_patch[e]]; n++)
							J[m * info->DIMENSION + l + 1] -= dR[n * info->DIMENSION + target_axis[l]] * info->Node_Coordinate[info->Controlpoint_of_Element[e * MAX_NO_CP_ON_ELEMENT + n] * (info->DIMENSION + 1) + m];
	...
	for (int i = 0; i < edge_n; i++)
	{
		for (int j = 0; j < face_n; j++)
		{
			int target_axis_base = 0;
			int target_axis[2] = {0, 0};
			int patch_base = info->Element_patch[td.e];
			int patch = info->Element_patch[td_base.e];
			double para_base[MAX_DIMENSION] = {0.0};
			double para[MAX_DIMENSION] = {0.0};
			double tilde_para_base[MAX_DIMENSION] = {0.0};
			double tilde_para[MAX_DIMENSION] = {0.0};
